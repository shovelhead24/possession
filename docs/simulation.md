...
- **Fixed in outcome, negotiable in tempo — PROPOSED 2026-07-29.** A fixed beat's *outcome* is not interceptable; its *timing* is. THE city falls either way, but a heavily-biased web of temporal interactions can be perturbed — the player can buy the city weeks, or hasten its end, without ever being able to prevent it. This gives the drumline real player agency without weakening it, and converts the relationship to doom from binary (save / fail) into **duration and cost**, which is both more playable and more honest to the fiction. Mechanically it needs nothing new: the beat is a scheduled fact, the tempo around it is clock state, and pre-biased dispositions (above) are exactly what perturbation acts on. Design obligation it creates: **both directions must have readable inputs** — prolonging and accelerating each need signals a player can find and act on, or the agency exists only in the simulation and not in the player's hands (`consumers.md`).
