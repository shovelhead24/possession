...
- **The vista is the anticipation engine, not decoration.** An empty stretch isn't empty if its far side shows something worth reaching. The distance must always promise — *and show a legible path* (the boat on the shore, the beam-tower stepping stones), not just a promise. Promise without a visible means is the "stranded on a plateau with no way down" failure. See pacing.md "Legible path, not just promise."
