...
func _terrain_h(arc: float, lat: float, w: float) -> float:
	...
	if _dem_w > 0:
		...
		if dh >= 0.0:
			return dh * dem_scale + wall_t * WALL_HEIGHT
...
func _rebuild() -> void:
	...
	# Near strip: real DEM when present (tighter + denser), else noise mountains
	if _dem_w > 0:
		_strip_arc = 42_000.0
		_strip_segs = 336
		_strip_rows = 132
		...
		_strip_segs = STRIP_SEGS
		_strip_rows = STRIP_ROWS
	st = SurfaceTool.new()
	st.begin(Mesh.PRIMITIVE_TRIANGLES)
	for i in _strip_segs + 1:
		var arc: float = _strip_arc * (2.0 * float(i) / float(_strip_segs) - 1.0)
		for j in _strip_rows + 1:
			var lat: float = w * (float(j) / float(_strip_rows) - 0.5)
			st.set_uv(Vector2(0.001, 0.5))  # off the city texture clusters
			st.add_vertex(_ring_pos(arc / r, lat, _terrain_h(arc, lat, w)))
	_grid_indices(st, _strip_segs, _strip_rows)
	st.generate_normals()
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event is InputEventKey and event.pressed:
		...
		match event.keycode:
			...
			KEY_H:
				if _dem_w > 0:
					dem_scale = 2.0 if dem_scale == 1.0 else (3.0 if dem_scale == 2.0 else 1.0)
					_rebuild()
		if _captured: return
